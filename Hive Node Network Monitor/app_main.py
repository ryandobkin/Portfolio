...
class AppMain:
    ...
    def run(self) -> None:
        ...
        parser.add_argument('-profile_id', type=int, default=AppSettings.DEFAULT_PROFILE_NAME, help='Profile to check for the local node')
        ...
        # Network Monitor
        network_monitor = NetworkMonitor(self.hive_node_manager, args.profile_id)
        network_monitor_thread = threading.Thread(target=network_monitor.run, daemon=True)
        ...
        # Blueprint Protocol Command Manager
        blueprint_protocol_command_manager = BlueprintProtocolCommandManager(self.hive_node_manager, self.outbound_message_queue)
        blueprint_protocol_command_manager_thread = threading.Thread(target=blueprint_protocol_command_manager.run, daemon=True)
        blueprint_protocol_command_manager_thread.start()
