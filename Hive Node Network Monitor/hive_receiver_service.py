...
class HiveReceiverService:
    ...
    def __init__(self, name: str, ip_address: str, port: int, hive_node_manager, inbound_message_queue: MessageQueue, outbound_message_queue: MessageQueue):
        ...
        outbound_message_queue : MessageQueue
            A queue for storing outgoing messages.
        hive_bp_manager : Dict
            A dictionary of {name:{service_check:param}} pairs.
        """
        ...
        with client_socket:
            while True:
                ...
                if command == 'connect':
                    ...
                    self.handle_blueprint(data_dict, sender_node)
                else:
    ...
    def handle_blueprint(self, incoming_message: Dict, sender_node: HiveNode) -> None:
        """
        Handles an incoming 'config' blueprint message, updating the existing config with any new profiles.
        Also checks update_id header, if set, update NetworkMonitor check ID

        Parameters:
        ----------
        blueprint_dict : Dict
            The dictionary containing a wrapper containing the blueprint list from the message.
        sender_node : HiveNode
            The node that sent the blueprint
        """
        self.logger.info("HiveReceiverService", f"Received blueprint from {sender_node.friendly_name}")
        # self.logger.info("HiveReceiverService", f"Old Blueprint: {self.hive_node_manager.blueprint_dict}")
        # self.logger.info("HiveReceiverService", f"Incoming message: {blueprint_dict}")
        # Before anything, check last_update attribute. Whoever was updated more recently wins the race
        if incoming_message['last_update'] >= self.hive_node_manager.last_update:

            # First, checks if needed to update NetworkMonitor ID, then set local outgoing list to new list
            hnm = self.hive_node_manager
            if hnm.local_node.friendly_name in incoming_message['update_id_dict']:
                new_outgoing = incoming_message['update_id_dict']
                hnm.incoming_update_check_id.update({hnm.local_node.friendly_name: new_outgoing[hnm.local_node.friendly_name]})
                new_outgoing.pop(hnm.local_node.friendly_name)
                hnm.outgoing_update_check_id = new_outgoing
                self.hive_node_manager.update_last_update()
            else:
                hnm.outgoing_update_check_id = incoming_message['update_id_dict']
                self.hive_node_manager.update_last_update()
            incoming_blueprint_dict = incoming_message['blueprint']

            # Second, check incoming config message against local dict
            for profile in incoming_blueprint_dict:
                # override local list with incoming list
                self.hive_node_manager.add_or_replace_service_profile(profile, incoming_blueprint_dict[profile])

            self.logger.info("HiveReceiverService", f"Handled Blueprint: Overwritten")
        else:
            self.logger.info("HiveReceiverService", f"Handled Blueprint: Discarded")

        # self.logger.info("HiveReceiverService", f"Handled blueprint from {sender_node.friendly_name}")
        # self.logger.info("HiveReceiverService", f"Updated Blueprint: {self.hive_node_manager.blueprint_dict}")
