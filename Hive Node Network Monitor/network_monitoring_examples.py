...
def check_server_http(url: str) -> Tuple[bool, Optional[int]]:
    ...
    """
    try:
        # Making a GET request to the server - added timeout as dns.google times out from http requests
        response: requests.Response = requests.get(url, timeout=1.5)
...
def check_server_https(url: str, timeout: int = 3) -> Tuple[bool, Optional[int], str]:
    """
...
def check_dns_server_status(server, query, record_type) -> (bool, str):
    ...
    # Having the timeout exception in lumped in as e caused
    # a giant string to be spit out on the line that wasn't readable.
    except dns.exception.Timeout:
        return False, "Timeout"

    except (dns.resolver.NoNameservers, dns.resolver.NoAnswer, socket.gaierror) as e:
...
def check_udp_port(ip_address: str, port: int, timeout: int = 1.5) -> (bool, str):
    """
